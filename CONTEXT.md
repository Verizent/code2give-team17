...
- [ ] **Add an approval / safeguarding gate before a just-in-time provisioned account can sign up for a first session.** Any Supabase signup currently becomes `role='volunteer'` on its first authenticated request, with no induction or screening — and classes include participants from age 6 (§20.5)
...
- [ ] **Confirm the Supabase Auth project settings the claim flow depends on:** *Confirm email* must be ON (otherwise signing up with a known volunteer's address is enough to inherit their hours and badges — `authenticate.js` refuses to link without `email_confirmed_at`, so the flow fails safe but also silently fails to link), and *Prevent user enumeration* must be ON (Supabase's default "already registered" error re-opens the oracle deliberately closed for volunteer email verification)

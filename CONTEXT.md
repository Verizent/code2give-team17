...
- [ ] Reconcile the two SQL homes (`supabase/migrations/` unapplied, `src/schema/` applied) into one, and de-duplicate `set_updated_at()` (§28). `src/schema/README.md` points at a `supabase/README.md` that does not exist, and calls `supabase/migrations/` "the applied migration history" when in fact it holds the content track's **unapplied** files — fix the pointer at the same time
- [ ] **Replace the hand-run admin grant with a role-management screen** plus an `audit_log` entry per grant. Today an admin is made by creating a user in the Supabase dashboard and running one `update public.profiles set role = 'admin'` (`src/schema/05_identity/90_grant_admin.example.txt`). There is no invite flow and no record of who promoted whom
- [ ] Remove the pre-created demo accounts and their throwaway credentials; issue real Love 21 staff accounts
- [ ] **Remove the shared admin account `admin@love21.demo`.** One credential the whole team knows, so no moderation action can be attributed to a person and the password lives in a chat message. Real use needs per-person admin accounts and an `audit_log` entry on every role change and every moderation decision
- [ ] **Add an approval / safeguarding gate before a newly provisioned account can sign up for a first session.** Any Supabase signup becomes `role='volunteer'` — written by the `on_auth_user_created` trigger at signup, with a just-in-time fallback in `authenticate.js` — with no induction or screening, and classes include participants from age 6 (§20.5)
- [ ] **Rate-limit authenticated admin mutations, and any auth endpoint that later exists.** This branch adds none — signup is browser→Supabase and provisioning is just-in-time — so there is currently nothing to limit. That changes the moment the email-verification endpoint is rebuilt, which without a per-address limit is an email-bombing tool (the note already says so in `20_email_verification/00_volunteer_email_verifications.sql`)
- [ ] **Rotate `volunteers.access_token` when a volunteer claims an account.** Today the tokenised link keeps working forever after the upgrade, so anyone who was ever forwarded it retains access to that volunteer's page indefinitely. One extra column in the existing claim `update` closes it
- [ ] Decide whether account-holding volunteers still need `volunteer_email_verifications` at all, or whether Supabase's own confirmation replaces it for that path — two proofs of the same fact currently coexist, and the table's logic is presently deleted
- [x] ~~*Confirm email* must be ON~~ — **verified ON, 2026-08-01.** The one real account was created `11:08:21` and confirmed `11:09:28`; with the toggle off Supabase writes both timestamps in the same statement and they are identical, so a 67-second gap is a human clicking a link. This matters because it is the only thing between typing a known volunteer's address into a signup form and inheriting their hours, badges and contact details — and the code reads identically either way, so it cannot be checked from the repo. Re-verify if anyone changes Auth settings
- [ ] ***Prevent user enumeration* must be ON** — still unconfirmed. Supabase's default "already registered" signup error re-opens the oracle deliberately closed for volunteer email verification, whose own note asks whether someone volunteers at a Down syndrome and autism charity is at least as sensitive
- [ ] **Revoke public EXECUTE on `public.handle_new_user()`.** It is a `security definer` function callable by `anon` and `authenticated` via `/rest/v1/rpc/handle_new_user`. Exploitability is low — it references `new` and errors outside trigger context — but it should not be reachable from the public API. `revoke execute on function public.handle_new_user() from anon, authenticated;` does not affect the trigger. Belongs to the track that owns the function
- [ ] Enable Supabase leaked-password protection (HaveIBeenPwned check), currently disabled
- [ ] **A revoked token stays valid for up to 60 seconds.** `verify-token.js` caches token→user for `CACHE_TTL_MS`. Deliberate — it is also what lets a role change take effect without a restart, and what throttles the volunteer claim to one attempt per token — but a real logout needs the cache invalidated, not just the session revoked
- [ ] **`adminGuard` is a convention, not a structural guarantee.** Admin surfaces are guarded per mount (`router.use("/api/admin/x", adminGuard, …)`), so a new admin route that omits it is reachable unauthenticated and looks exactly like its neighbours. `tests/routes/admin-mount.test.js` fails if any mount drops it — keep that test, or restore a single gated parent router

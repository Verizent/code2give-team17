...
const BAR_COLORS = ['bg-teal', 'bg-pink', 'bg-yellow', 'bg-navy'] as const
...
export function WishlistGrid() {
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      ...
      const result = await pledgeWishlistItem(selected.id, {
        name,
        email,
        quantity: qty,
      })
      trackEvent('wishlist_pledge', { id: selected.id, quantity: qty })
      setItems((current) =>
        current.map((item) => (item.id === selected.id ? result.item : item)),
      )
      setSent(true)
    } catch (err) {
  ...
  if (loading) {
    return <p className="text-navy/70">{g.wishlistSubhead}</p>
  }
            ...
            {sent ? (
              ...
              <form onSubmit={submit} className="mt-6 space-y-4">
                ...
                <button
                  type="submit"
                  disabled={submitting}
                  className="min-h-12 w-full rounded-md bg-red px-5 font-semibold text-white disabled:opacity-60"
                >
                  {submitting ? '…' : g.pledgeSubmit}
                </button>
              </form>
            )}
          </div>

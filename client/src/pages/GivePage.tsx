...
export function GivePage() {
  ...
  const [publicCampaigns, setPublicCampaigns] = useState<LocalCampaign[]>([])
  const [myCampaigns, setMyCampaigns] = useState<LocalCampaign[]>([])
  ...
  useEffect(() => {
    ...
    listCampaigns()
      .then(({ public: live, mine }) => {
        setPublicCampaigns(live)
        setMyCampaigns(mine)
      })
      .catch(() => {
        setPublicCampaigns([])
      ...
      <main id="main">
        ...
        <div className="mx-auto max-w-[1120px] px-4 py-10 sm:px-8 sm:py-14">
          {tab === 'money' && (
            <>
              {donateError && (
                <p role="alert" className="mb-6 rounded-xl bg-red/10 px-4 py-3 text-sm text-red">
                  {donateError}
                </p>
              )}
          ...
          {tab === 'fundraise' && (
            <section>
              ...
              {publicCampaigns.length ? (
                ...
                <p className="mt-5 rounded-2xl bg-white p-5 text-navy/70">{g.noPublicCampaigns}</p>
              )}
              ...
              {myCampaigns.length ? (
                <div className="mt-5 grid gap-4 sm:grid-cols-2">
                  {myCampaigns.map((campaign) => (
                    <CampaignCard key={campaign.slug} campaign={campaign} showStatus />
                  ))}
                </div>
              ) : (
...
function CampaignCard({
  ...
  return (
    <Link to={`/c/${campaign.slug}`} className="flex gap-4 rounded-2xl bg-white p-4">
      <img src={campaign.cover} alt="" className="h-24 w-28 rounded-xl object-cover" />
      <div>
        {showStatus && campaign.status === 'pending_approval' && (
          <span className="mb-2 inline-flex rounded-md bg-amber px-2 py-0.5 text-xs font-semibold text-navy">
            Pending
          </span>
        )}
        {showStatus && campaign.status === 'rejected' && (
          <span className="mb-2 inline-flex rounded-md bg-red/10 px-2 py-0.5 text-xs font-semibold text-red">
            Rejected
          </span>
        )}
        <p className="font-display text-lg font-semibold text-navy">{campaign.title}</p>
        <p className="mt-2 text-sm text-navy/65">
          HK${campaign.raised_hkd.toLocaleString()} / HK${campaign.goal_hkd.toLocaleString()}
        </p>
      </div>
    </Link>
  )
